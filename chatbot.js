class ChatBot extends HTMLElement {
    constructor() {
        ...
        this.chatSession = this.getCookie('chatSession');
    ...
    async connectedCallback() {
        try {
            ...
            if (this.chatbotData.transcript) {
                ...
                this.messages.push({
                    role: 'assistant',
                    content: `Welcome to the chat! I am ${this.chatbotData?.customName || "Bibi"}, the virtual assistent🤖 of this webstore. I am able to handle most of your question, feel free to ask your first question or choose one of the suggestions below!`,
                });
                ...
                this.messages.push({
                    role: 'assistant',
                    content: `Welcome to ${this.chatbotData?.shop.customName || "our store"}! Fill in your email address to start a chat.`,
                });

                if (!this.chatStarted) {
                    const emailInputSection = `
                    <div class="email-input">
                        <input type="email" name="email" autocomplete="email" placeholder="E-mailaddress">
    ...
    async fetchChatbotData() {
        try {
            const response = await fetch('https://your-app-domain/serve', {
                method: 'POST',
                headers: {
    ...
    async startChat(email) {
        try {
            ...
            const response = await fetch('https://your-app-domain/chatToken', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            if (data.token) {
                document.cookie = `chatSession = ${data.token}; path = /; max-age=3600`;
        ...
        this.shadowRoot.innerHTML = `
            ...
            <div class="chat-window">
                <div class="chat-header">
                    <h4>${this.chatbotData?.shop.customName || "Customer Support"}</h4>
                    <span>Chatting with ${this.chatbotData?.customName || "Bibi"}</span>
                </div>
                <div class="chat-log"></div>
                <div class="chat-input">
                    <input name="question" type="text" placeholder="Ask anything...">
    ...
    async sendMessage(message) {
        ...
        try {
            const response = await fetch('https://your-app-domain/api/assistant/chat', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
